...
tasks.register("generateCiConfigProperties") {
    description = "Generates a committed CI config asset if missing"
    group = "build setup"

    doLast {
        val ciConfigFile = file("src/main/assets/config.properties.ci")
        val templateFile = file("src/main/assets/config.properties.template")

        if (!ciConfigFile.exists() && templateFile.exists()) {
            ciConfigFile.parentFile.mkdirs()
            templateFile.copyTo(ciConfigFile)
            println("Generated config.properties.ci from template for CI build")
        }
    }
}

tasks.named("preBuild") {
    dependsOn("generateCiConfigProperties")
}
